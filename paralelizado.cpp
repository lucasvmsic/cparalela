...
bool crearArchivosMensuales(const string& archivoEntrada, map<string, ofstream>& archivosMensuales) {
    ...
    #pragma omp parallel private(linea, acumulador, acumulando)
    {
...
int main() {
    ...
    if (!crearArchivosMensuales("2millones.csv", archivosMensuales)) {
        cerr << "Error al crear los archivos mensuales." << endl;
        return 1;
    }

    float inflacionAcumulada = 0.0;
	#pragma omp parallel for reduction(+:inflacionAcumulada)
    for (size_t i = 1; i < meses.size(); ++i) {
		...
		#pragma omp parallel for reduction(+:sumaPreciosMesActual, sumaPreciosMesAnterior)
